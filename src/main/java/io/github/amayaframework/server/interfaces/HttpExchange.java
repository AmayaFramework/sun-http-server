 ...
 * <li>{@link #sendResponseHeaders(HttpCode, long)} to send the response headers. Must be called before
 * next step.
 ...
 * Closing an exchange without consuming all the request body is not an error
...
public interface HttpExchange {
     ...
     * It is recommended that applications should consume (read) all the
     ...
     * written. {@link #sendResponseHeaders(HttpCode, long)}) must be called prior to calling
     ...
     * @param responseLength if more than 0, specifies a fixed response body length
     ...
     * required to be) subclasses of {@link java.io.FilterInputStream}
     * and {@link java.io.FilterOutputStream}.
     *
     * @param inputStream  the filtered input stream to set as this object's input stream,
     *                     or <code>null</code> if no change.
     * @param outputStream the filtered output stream to set as this object's output stream,
     *                     or <code>null</code> if no change.
     */
